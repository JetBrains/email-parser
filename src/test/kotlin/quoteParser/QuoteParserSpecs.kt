...
class QuoteParserSpecs : Spek() {
    init {
        given("QuoteParser.parse() function") {
...
> мыслей, я ими поделюсь, а дальше будем думать :)"""
                ...
                val expected = "15 апреля 2016 г., 20:26 пользователь Mariya Davydova <\n" +
                        "mariya.davydova@jetbrains.com> написал:"
                val actual = QuoteParser(text.lines()).parse().header.toString()
...
лифтовой площадки)."""
                ...
                it("should return null") {
                    assertNull(QuoteParser(text.lines()).parse().header)
                }
            }
        }
    }
